...
+ (DateTimeView*)getInstanceWithNibWithBlock:(ClickTimeDoneBlock)aBlock{
    DateTimeView *view = nil;
    NSArray *objs = [[NSBundle mainBundle] loadNibNamed:@"DateTimeView" owner:nil options:nil];
    for(id obj in objs) {
        if([obj isKindOfClass:[DateTimeView class]]){
            view = (DateTimeView *)obj;
            break;
        }
    }
...
- (IBAction)actionDone:(id)sender
{
    ...
    NSString *str =[NSString stringWithFormat:@"%@-%@",[self.yearArray objectAtIndex:[self.customPicker selectedRowInComponent:0]],[self.monthArray objectAtIndex:[self.customPicker selectedRowInComponent:1]]];
    if (_clickDoneBlock) {
        _clickDoneBlock(str);
    }
     [self hide];
