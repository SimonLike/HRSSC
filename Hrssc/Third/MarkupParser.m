...
- (id)init
{
    ...
    if (self) {
        ...
        self.font = _textFont.fontName;//@"ArialMT";
...
-(NSAttributedString *)attrStringFromMarkup:(NSString *)markup
{
    ...
    for (NSTextCheckingResult *b in chunks) {
        ...
        CTFontRef fontRef = CTFontCreateWithName((CFStringRef)_textFont.fontName, _textFont.pointSize, NULL);//lxf self.font 24.0
        ...
        if ([parts count] > 1) {
            ...
            if ([tag hasPrefix:@"font"]) {
                ...
                [colorRegex enumerateMatchesInString:tag options:0 range:NSMakeRange(0, [tag length]) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
                    SEL colorSel = NSSelectorFromString([NSString stringWithFormat:@"%@",[tag substringWithRange:result.range]]);
                    if (colorSel == @selector(colorWithRed)) {
                        self.color = [UIColor colorWithRed:76/255.0 green:76/255.0 blue:76/255.0 alpha:1.0];
                    }
                    else
