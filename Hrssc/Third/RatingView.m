...
//    height *=2.;
    ...
    CGFloat gapWidth = gapStar + width;//lxf
    
	[s1 setFrame:CGRectMake(0,         0, width, height)];
	[s2 setFrame:CGRectMake(gapWidth,     0, width, height)];
	[s3 setFrame:CGRectMake(2 * gapWidth, 0, width, height)];
	[s4 setFrame:CGRectMake(3 * gapWidth, 0, width, height)];
	[s5 setFrame:CGRectMake(4 * gapWidth, 0, width, height)];
	...
	frame.size.width = gapWidth * 5;//lxf
...
-(void) touchesMoved: (NSSet *)touches withEvent: (UIEvent *)event
{
	...
	int newRating = (int) (pt.x / (width+gapStar)) + 1;
	if (newRating < 1 || newRating > 5)
		return;
	//改变newRating的类型就改变是否显示半星
