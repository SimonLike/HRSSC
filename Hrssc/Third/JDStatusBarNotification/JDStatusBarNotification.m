                ...
                styleName:(NSString*)styleName;
{
    UIView *view = [self showWithStatus:status
                              styleName:styleName];
    UIButton *btn = [[UIButton alloc] initWithFrame:[self sharedInstance].topBar.bounds];
    [btn addTarget:self action:@selector(dismissw) forControlEvents:UIControlEventTouchUpInside];
    [[self sharedInstance].topBar addSubview:btn];
...
- (void)setupDefaultStyles;
{
    ...
    [self addStyleNamed:JDStatusBarStyleDark
                prepare:^JDStatusBarStyle *(JDStatusBarStyle *style) {
                    style.barColor = [Utils colorWithHexString:@"A58DE1"];
